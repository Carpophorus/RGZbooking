(function(global) {
  ...
  //RGZ.frontRoot = 'http://93.87.56.76:8084';
  ...
  var generateBookHtml = function() {
    ...
    bookHtml += `
          ...
          <label class="checkbox-label" onclick="$RGZ.checkboxClicked($('#book-counter-check'));">Потврђујем да ћу предати потпуну документацију у прилогу захтева за упис због којег заказујем термин за предају захтева. У случају кашњења или доношења непотпуне документације, пристајем да мој захтев не буде запримљен и да наредна странка буде услужена. Прихватам и ограничење да на шалтеру Службе нећу вршити измену или допуне поднете пријаве за заказивање и <strong>да ћу поднети само један захтев</strong>. Пристајем да захтев не буде запримљен на лицу места уколико немам потпуну документацију за подношење истог.</label>
    ...
    if (n == 2 && nav != 2) {
      ...
      if (!statusDescViewed)
        $.confirm({
          ...
          content: 'Овај део апликације еЗаказивање Вам помаже да проверите статус свог предмета, без потребе да долазите до одговарајуће Службе Катастра Непокретности или да зовете Инфо Центар.<br><br><div style="width: 100%; text-align: left">&bull;&nbsp;Одаберите жељену Службу Катастра Непокретности и упишите број Вашег предмета (поље подвучено црвеном линијом прихвата податке у формату "nnnn" или "sss-nnnn", где је "nnnn" број предмета, а "sss" број Службе по новој класификацији). Потом, кликните на дугме ПРЕТРАГА.<br>&bull;&nbsp;Радни назив статуса Вашег предмета ће се приказати након провере података.</div>',
          theme: 'supervan',
          buttons: {
            ok: {
              text: 'ОК',
